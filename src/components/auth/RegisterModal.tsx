...
export default function RegisterModal({
  ...
  const [loadingProvider, setLoadingProvider] =
    useState<null | "google" | "apple">(null);
  ...
  function updateProgress(next = form) {
    let p = 0;
    if (next.name.length > 2) p++;
    if (next.email.length > 4 && next.email.includes("@")) p++;
    if (next.password.length >= 6) p++;
    if (next.password === next.confirm && next.confirm.length > 5) p++;
    setProgress(p);
  }
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="relative">
            ...
            {form.confirm && (
              <span
                className={`absolute right-10 top-2.5 text-lg transition
                  ${
