...
export default function CommentsSection({ postId, authorId }: Props) {
  const { user } = useUser();
 ...
 useEffect(() => {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE_URL}/blogs/${postId}/comments?sort=${sortMode}`,
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editing) {
        const res = await fetch(
          `${API_BASE_URL}/blogs/${postId}/comments/${editing.id}`,
          {
            method: "PUT",
            headers: {
        ...
        const res = await fetch(`${API_BASE_URL}/blogs/${postId}/comments`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
  ...
  const handleTogglePin = async (c: CommentNode) => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE_URL}/blogs/${postId}/comments/${c.id}/pin`,
        {
          method,
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleDelete = async (c: CommentNode) => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/blogs/${postId}/comments/${c.id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
 ...
 const handleToggleLike = async (c: CommentNode) => {
  ...
  try {
    const res = await fetch(
      `${API_BASE_URL}/blogs/${postId}/comments/${c.id}/like`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
